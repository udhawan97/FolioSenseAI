...
Explains why a holding moved today using market data, volume, and earnings context.
...
def _attribute(  # pylint: disable=too-many-positional-arguments,too-many-return-statements
    ...
    if near_earnings:
        return "earnings-driven", "High" if high_vol else "Medium"
...
def explain_move(  # pylint: disable=too-many-branches,too-many-statements
    ...
    # ── Individual stock ──────────────────────────────────────────────────────
        ...
        attribution_type, confidence = _attribute(
            day_chg_pct, spy_chg, sector_chg, high_vol, near_earnings
        )
        ...
        if not high_vol and not near_earnings and attribution_type not in (
